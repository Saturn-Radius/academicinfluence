...
export default async function serveDisciplines(request: DisciplinesRequest): Promise<DisciplinesResponse> {
    ...
    const superdisciplineQuery = pool.query(squel.select().from("editor.ai_disciplines")
        .field("editor.ai_disciplines.name")
        .toParam())

    const subdisciplineQuery = pool.query(squel.select().from("editor.ai_subdisciplines")
        .field("editor.ai_subdisciplines.name", "name")
        .field("editor.ai_disciplines.name", "parent")
        .join("editor.ai_disciplines", undefined, "editor.ai_disciplines.id = editor.ai_subdisciplines.parent")
        .toParam())
